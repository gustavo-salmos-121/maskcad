...
function renderForSim(ctx: CanvasRenderingContext2D, shapes: Shape[], dW: number, dH: number, cW: number, cH: number, pol: Polarity) {
  ...
  shapes.forEach(s => { if(s.type==='rect') ctx.fillRect(s.x*cW/dW,s.y*cH/dH,s.w*cW/dW,s.h*cH/dH); else{ctx.beginPath();ctx.arc(s.cx*cW/dW,s.cy*cH/dH,s.r*cW/dW,0,Math.PI*2);ctx.fill();} });
}
