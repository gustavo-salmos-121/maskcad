...
function renderOneDie(ctx: CanvasRenderingContext2D, shapes: Shape[], dx: number, dy: number, dw: number, dh: number, marks: MarksConfig, scale: number, fgColor: string, footprintW: number, footprintH: number) {
  ...
  ctx.save(); ctx.beginPath(); ctx.rect(Math.floor(dx-pad), Math.floor(dy-pad), Math.ceil(footprintW), Math.ceil(footprintH)); ctx.clip();
  ctx.fillStyle = fgColor;
  shapes.forEach(s => {
    if (s.type==='rect') {
      const px=Math.floor(dx+s.x*scale), py=Math.floor(dy+s.y*scale);
      const px2=Math.ceil(dx+(s.x+s.w)*scale), py2=Math.ceil(dy+(s.y+s.h)*scale);
      ctx.fillRect(px, py, px2-px, py2-py);
    } else {
      ctx.beginPath(); ctx.arc(Math.round(dx+s.cx*scale), Math.round(dy+s.cy*scale), Math.ceil(s.r*scale), 0, Math.PI*2); ctx.fill();
    }
  });
...
function renderForSim(ctx: CanvasRenderingContext2D, shapes: Shape[], dW: number, dH: number, cW: number, cH: number, pol: Polarity) {
  ...
  // Disable anti-aliasing — we need hard pixel edges for binary masks
  ...
  shapes.forEach(s => {
    if (s.type === 'rect') {
      // Snap to pixel grid: floor the start, ceil the end → no sub-pixel gaps
      const px = Math.floor(s.x * sx);
      const py = Math.floor(s.y * sy);
      const px2 = Math.ceil((s.x + s.w) * sx);
      const py2 = Math.ceil((s.y + s.h) * sy);
      ctx.fillRect(px, py, px2 - px, py2 - py);
    } else {
      // Circles: round center, ceil radius
      const cx = Math.round(s.cx * sx);
      const cy = Math.round(s.cy * sy);
      const r = Math.ceil(s.r * sx);
