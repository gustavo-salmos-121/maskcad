...
import { Shape, Layer, Viewport, Tool, Polarity, MarksConfig, HistoryEntry, ProjectData, WAFER_SIZES, LAYER_PALETTE, DEFAULT_MARKS, scaleShapes, polarArray, mirrorCopy, linearArray, getGroupBounds } from './types';
import { exportSimMEMS, exportSimMEMSPerLayer, exportWafer, saveProject, loadProject, visibleShapes } from './utils';
import Toolbar from './components/Toolbar';
import Canvas from './components/Canvas';
import RightPanel from './components/RightPanel';
import WaferModal from './components/WaferModal';

const CY='2025',CH='SyncField Corporation',AV='2.1.0';
...
function MaskCADApp(){
  ...
  const push=useCallback((l:string)=>{setHistory(p=>{const t=[...p.slice(0,hi+1),{shapes:JSON.parse(JSON.stringify(shapes)),label:l}];if(t.length>MH)t.shift();return t;});setHi(p=>Math.min(p+1,MH-1));},[shapes,hi]);
  ...
  const doGroup=useCallback(()=>{if(selectedIds.length<2)return;const gid=nextGroupId;setShapes(p=>p.map(s=>selectedIds.includes(s.id)?{...s,groupId:gid}:s));setNextGroupId(p=>p+1);push('Agrupar');},[selectedIds,nextGroupId,push]);
  const doUngroup=useCallback(()=>{const gids=new Set(selShapes.filter(s=>s.groupId!=null).map(s=>s.groupId!));if(!gids.size)return;setShapes(p=>p.map(s=>gids.has(s.groupId!)?{...s,groupId:undefined}:s));push('Desagrupar');},[selShapes,push]);
  const doScale=useCallback((f:number)=>{if(!selectedIds.length||f===1)return;setShapes(p=>scaleShapes(p,selectedIds,f));push(`Escalar ×${f}`);},[selectedIds,push]);

  const doPolarArray=useCallback((cx:number,cy:number,count:number,totalAngle:number)=>{
    if(!selectedIds.length||count<2)return;
    const r=polarArray(shapes,selectedIds,cx,cy,count,totalAngle,nextId);
    setShapes(p=>[...p,...r.newShapes]);setNextId(r.nextId);
    setSelectedIds([...selectedIds,...r.newShapes.map(s=>s.id)]);push(`Array polar ×${count}`);
  },[shapes,selectedIds,nextId,push]);

  const doLinearArray=useCallback((cols:number,rows:number,sx:number,sy:number)=>{
    if(!selectedIds.length||(cols*rows<=1))return;
    const r=linearArray(shapes,selectedIds,cols,rows,sx,sy,nextId);
    setShapes(p=>[...p,...r.newShapes]);setNextId(r.nextId);
    setSelectedIds([...selectedIds,...r.newShapes.map(s=>s.id)]);push(`Array ${cols}×${rows}`);
  },[shapes,selectedIds,nextId,push]);

  const doMirrorX=useCallback(()=>{
    if(!selectedIds.length)return;
    const b=getGroupBounds(shapes,selectedIds);
    const r=mirrorCopy(shapes,selectedIds,'y',b.cx,b.cy,nextId);
    setShapes(p=>[...p,...r.newShapes]);setNextId(r.nextId);
    setSelectedIds([...selectedIds,...r.newShapes.map(s=>s.id)]);push('Espelhar ↔');
  },[shapes,selectedIds,nextId,push]);

  const doMirrorY=useCallback(()=>{
    if(!selectedIds.length)return;
    const b=getGroupBounds(shapes,selectedIds);
    const r=mirrorCopy(shapes,selectedIds,'x',b.cx,b.cy,nextId);
    setShapes(p=>[...p,...r.newShapes]);setNextId(r.nextId);
    setSelectedIds([...selectedIds,...r.newShapes.map(s=>s.id)]);push('Espelhar ↕');
  },[shapes,selectedIds,nextId,push]);
  ...
  useEffect(()=>{
    const h=(e:KeyboardEvent)=>{
      ...
      if((e.ctrlKey||e.metaKey)&&(e.key==='G'||(e.key==='g'&&e.shiftKey))){e.preventDefault();doUngroup();return;}
      ...
      if(!e.ctrlKey&&!e.metaKey){
        ...
        if(e.key==='p')setTool('polygon');if(e.key==='a')setTool('arc');
        if(e.key==='v')setTool('select');if(e.key==='g')setShowGrid(p=>!p);
      }
    };window.addEventListener('keydown',h);return()=>window.removeEventListener('keydown',h);
  },[selectedIds,undo,redo,push,doGroup,doUngroup,selectAll]);

  const dup=useCallback(()=>{
    if(!selectedIds.length)return;const ns:Shape[]=[];let nid=nextId;
    selectedIds.forEach(sid=>{const s=shapes.find(sh=>sh.id===sid);if(!s)return;
      const n={...s,id:nid,groupId:undefined} as Shape;
      if(n.type==='rect'){n.x+=gridSize;n.y+=gridSize;}
      if(n.type==='circle'){n.cx+=gridSize;n.cy+=gridSize;}
      if(n.type==='polygon'){(n as any).points=(s as any).points.map((p:any)=>({x:p.x+gridSize,y:p.y+gridSize}));}
      if(n.type==='arc'){n.cx+=gridSize;n.cy+=gridSize;}
      ns.push(n);nid++;});
    setShapes(p=>[...p,...ns]);setSelectedIds(ns.map(s=>s.id));setNextId(nid);push('Duplicar');
  ...
  const doSave=useCallback(()=>saveProject({version:5,name:projectName,shapes,layers,deviceW,deviceH,gridSize,polarity,simPxW,simPxH,waferDiam,streetW,marks,waferDPI}),[projectName,shapes,layers,deviceW,deviceH,gridSize,polarity,simPxW,simPxH,waferDiam,streetW,marks,waferDPI]);
  const doLoad=useCallback(async(f:File)=>{try{const d=await loadProject(f);setShapes(d.shapes||[]);setLayers(d.layers||DL);setDeviceW(d.deviceW||500);setDeviceH(d.deviceH||500);setGridSize(d.gridSize||25);setPolarity(d.polarity||'dark');setSimPxW(d.simPxW||256);setSimPxH(d.simPxH||256);setWaferDiam(d.waferDiam||100000);setStreetW(d.streetW||200);setMarks(d.marks||DEFAULT_MARKS);setWaferDPI(d.waferDPI||2540);setProjectName(d.name||'Carregado');setSelectedIds([]);setNextId(Math.max(...(d.shapes||[]).map(s=>s.id),0)+1);setNextGroupId(Math.max(...(d.shapes||[]).map(s=>s.groupId||0),0)+1);setHistory([{shapes:JSON.parse(JSON.stringify(d.shapes||[])),label:'Carregar'}]);setHi(0);}catch(e){alert('Erro: '+(e as Error).message);}},[]);
  ...
  return<div style={{width:'100%',height:'100vh',display:'flex',flexDirection:'column',background:'var(--bg)',overflow:'hidden',userSelect:'none'}}>
    ...
    <div style={{display:'flex',flex:1,overflow:'hidden'}}>
      <Toolbar tool={tool} setTool={setTool} hasSelection={selectedIds.length>0} selectionCount={selectedIds.length}
        ...
        onMirrorX={doMirrorX} onMirrorY={doMirrorY} onExportSimMEMS={exSim}/>
      ...
      <RightPanel deviceW={deviceW} setDeviceW={setDeviceW} deviceH={deviceH} setDeviceH={setDeviceH}
        ...
        shapes={shapes} setShapes={setShapes} layers={layers} setLayers={setLayers}
        ...
        onScale={doScale} onPolarArray={doPolarArray} onLinearArray={doLinearArray} pushHistory={push}/>
    </div>
    ...
    {aboutOpen&&<div style={{position:'fixed',inset:0,background:'var(--modal-bg)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000,backdropFilter:'blur(6px)'}} onClick={()=>setAboutOpen(false)}>
      <div className="fade-in" onClick={e=>e.stopPropagation()} style={{background:'var(--panel)',borderRadius:12,border:'1px solid var(--border)',padding:32,maxWidth:420,width:'90vw',boxShadow:'0 24px 64px rgba(0,0,0,0.4)',textAlign:'center'}}>
        ...
        <p style={{fontSize:13,color:'var(--text)',lineHeight:1.7}}>Editor vetorial de máscaras litográficas para semicondutores e MEMS.</p>
        <p style={{fontSize:12,color:'var(--text-dim)',lineHeight:1.6,marginTop:8}}>Polígonos · Arcos · Array polar e linear · Espelhamento · Exportação simMEMS e fotolito</p>
