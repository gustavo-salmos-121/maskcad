...
interface CanvasProps {
  ...
  activeLayer: number; nextId: number;
  setNextId: React.Dispatch<React.SetStateAction<number>>;
  onShapeCreated: () => void; onShapeMoved: () => void;
  theme: string;
}

interface DrawingState {
  type: 'rect'|'circle'; startX: number; startY: number;
...
function drawGridLines(ctx: CanvasRenderingContext2D, vp: Viewport, gridSize: number, W: number, H: number, minor: string, major: string) {
  const [wl,wt]=s2w(0,0,vp,W,H),[wr,wb]=s2w(W,H,vp,W,H);
  const sx=Math.floor(wl/gridSize)*gridSize, sy=Math.floor(wt/gridSize)*gridSize;
  for(let x=sx;x<=wr;x+=gridSize){const[px]=w2s(x,0,vp,W,H);const m=Math.abs(x%(gridSize*5))<0.01;ctx.strokeStyle=m?major:minor;ctx.lineWidth=m?1:0.5;ctx.beginPath();ctx.moveTo(px,0);ctx.lineTo(px,H);ctx.stroke();}
  for(let y=sy;y<=wb;y+=gridSize){const[,py]=w2s(0,y,vp,W,H);const m=Math.abs(y%(gridSize*5))<0.01;ctx.strokeStyle=m?major:minor;ctx.lineWidth=m?1:0.5;ctx.beginPath();ctx.moveTo(0,py);ctx.lineTo(W,py);ctx.stroke();}
}
...
export default function Canvas({
  ...
  useEffect(()=>{
    ...
    ctx.fillStyle=deviceBg;ctx.fillRect(dx0,dy0,dw,dh);
    ...
    shapes.forEach(s=>{
      ...
      // Group indicator: dashed border
      if(hasGroup&&!isSel){ctx.setLineDash([4,3]);ctx.lineWidth=1.5;}

      if(s.type==='rect'){const[rx,ry]=w2s(s.x,s.y,vp,W,H);const rw=s.w*vp.z,rh=s.h*vp.z;ctx.fillRect(rx,ry,rw,rh);ctx.strokeRect(rx,ry,rw,rh);
      ...
      } else {const[cx,cy]=w2s(s.cx,s.cy,vp,W,H);const sr=s.r*vp.z;ctx.beginPath();ctx.arc(cx,cy,sr,0,Math.PI*2);ctx.fill();ctx.stroke();
    ...
    // Group bounding box when multiple selected
    if(selectedIds.length>1){
      let gMinX=Infinity,gMinY=Infinity,gMaxX=-Infinity,gMaxY=-Infinity;
      shapes.filter(s=>selSet.has(s.id)).forEach(s=>{
        if(s.type==='rect'){const[rx,ry]=w2s(s.x,s.y,vp,W,H);gMinX=Math.min(gMinX,rx);gMinY=Math.min(gMinY,ry);gMaxX=Math.max(gMaxX,rx+s.w*vp.z);gMaxY=Math.max(gMaxY,ry+s.h*vp.z);}
        else{const[cx,cy]=w2s(s.cx,s.cy,vp,W,H);const sr=s.r*vp.z;gMinX=Math.min(gMinX,cx-sr);gMinY=Math.min(gMinY,cy-sr);gMaxX=Math.max(gMaxX,cx+sr);gMaxY=Math.max(gMaxY,cy+sr);}
      });
      ctx.strokeStyle=accent;ctx.lineWidth=1;ctx.setLineDash([6,4]);
      ctx.strokeRect(gMinX-4,gMinY-4,gMaxX-gMinX+8,gMaxY-gMinY+8);
      ctx.setLineDash([]);
      ctx.fillStyle=accent;ctx.font='10px "JetBrains Mono",monospace';
      ctx.fillText(`${selectedIds.length} formas selecionadas`,gMinX,gMinY-8);
    }

    if(drawing){
      ctx.fillStyle=previewFill;ctx.strokeStyle=accent;ctx.lineWidth=1.5;ctx.setLineDash([6,4]);
      if(drawing.type==='rect'&&drawing.w!==undefined){const[rx,ry]=w2s(drawing.x!,drawing.y!,vp,W,H);ctx.fillRect(rx,ry,drawing.w!*vp.z,drawing.h!*vp.z);ctx.strokeRect(rx,ry,drawing.w!*vp.z,drawing.h!*vp.z);}
      else if(drawing.type==='circle'&&drawing.r!==undefined){const[cx,cy]=w2s(drawing.cx!,drawing.cy!,vp,W,H);ctx.beginPath();ctx.arc(cx,cy,drawing.r!*vp.z,0,Math.PI*2);ctx.fill();ctx.stroke();}
      ctx.setLineDash([]);
    }
  ...
  },[shapes,vp,cSize,showGrid,gridSize,selectedIds,drawing,deviceW,deviceH,polarity,layers,theme]);
  ...
  const handleMouseDown=useCallback((e:React.MouseEvent)=>{
    e.preventDefault();
    const{wx,wy,rawX,rawY,sx,sy}=getWP(e);
    ...
    if(tool==='select'){
      ...
      if(hit){
        // If hit shape is in a group, select all group members
        let newIds: number[];
        const groupShapeIds = hit.groupId != null
          ? shapes.filter(s => s.groupId === hit.groupId).map(s => s.id)
          : [hit.id];

        if(e.shiftKey){
          // Shift: toggle selection
          const current=new Set(selectedIds);
          const allIn=groupShapeIds.every(id=>current.has(id));
          if(allIn) groupShapeIds.forEach(id=>current.delete(id));
          else groupShapeIds.forEach(id=>current.add(id));
          newIds=Array.from(current);
        } else {
          newIds=groupShapeIds;
        }
      ...
      setDrawing({type:'rect',startX:wx,startY:wy,x:wx,y:wy,w:0,h:0});
    } else if(tool==='circle'){
      setDrawing({type:'circle',startX:wx,startY:wy,cx:wx,cy:wy,r:0});
    }
  ...
  const handleMouseMove=useCallback((e:React.MouseEvent)=>{
    ...
    if(dragging){
      ...
      setShapes(prev=>prev.map(s=>{
        const orig=dragging.origShapes.find((o:Shape)=>o.id===s.id);
        if(!orig) return s;
        if(s.type==='rect') return{...s,x:orig.x+dx,y:orig.y+dy};
        return{...s,cx:orig.cx+dx,cy:orig.cy+dy};
      }));
      return;
    }
    if(drawing){
  ...
  },[drawing,dragging,panning,vp,getWP,gridSize,snapOn]);
  ...
  const handleMouseUp=useCallback(()=>{
    ...
    if(drawing){
      ...
      if(ns){setShapes(p=>[...p,ns!]);setNextId(p=>p+1);setSelectedIds([ns.id]);onShapeCreated();}
      setDrawing(null);
    }
  ...
  return <div ref={containerRef} style={{flex:1,position:'relative',overflow:'hidden'}}>
    ...
    {al&&<div style={{position:'absolute',top:8,left:10,display:'flex',alignItems:'center',gap:6,fontSize:11,color:'var(--text-dim)',background:'var(--overlay-bg)',padding:'4px 10px',borderRadius:'var(--radius)',backdropFilter:'blur(8px)'}}>
      ...
      {selectedIds.length>1&&<span style={{color:'var(--accent)',marginLeft:8}}>· {selectedIds.length} selecionadas</span>}
    </div>}
    <div style={{position:'absolute',bottom:8,left:10,display:'flex',gap:16,fontSize:11,fontFamily:'var(--font-mono)',color:'var(--text-dim)',background:'var(--overlay-bg)',padding:'4px 12px',borderRadius:'var(--radius)',backdropFilter:'blur(8px)'}}>
      <span>X: {mouseWorld.x.toFixed(1)} µm</span><span>Y: {mouseWorld.y.toFixed(1)} µm</span>
      <span>Zoom: {(vp.z*100).toFixed(0)}%</span><span>Formas: {shapes.length}</span>
    </div>
  </div>;
}
