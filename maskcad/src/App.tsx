...
import { Shape, Layer, Viewport, Tool, Polarity, MarksConfig, HistoryEntry, ProjectData, WAFER_SIZES, LAYER_PALETTE, DEFAULT_MARKS } from './types';
...
const COPYRIGHT_HOLDER = 'SyncField Corporation';
...
function MaskCADApp(){
  ...
  const undo=useCallback(()=>{if(hi<=0)return;setShapes(JSON.parse(JSON.stringify(history[hi-1].shapes)));setHi(hi-1);setSelectedId(null);},[history,hi]);
  const redo=useCallback(()=>{if(hi>=history.length-1)return;setShapes(JSON.parse(JSON.stringify(history[hi+1].shapes)));setHi(hi+1);setSelectedId(null);},[history,hi]);
  ...
  useEffect(()=>{
    const h=(e:KeyboardEvent)=>{
      ...
      if(e.key==='Delete'||e.key==='Backspace'){if(selectedId!=null){setShapes(p=>p.filter(s=>s.id!==selectedId));setSelectedId(null);push('Excluir');}}
      if(e.key==='Escape')setTool('select');if(e.key==='r')setTool('rect');if(e.key==='c')setTool('circle');if(e.key==='v')setTool('select');if(e.key==='g')setShowGrid(p=>!p);
    };window.addEventListener('keydown',h);return()=>window.removeEventListener('keydown',h);
  },[selectedId,undo,redo,push]);

  const dup=useCallback(()=>{
    const s=shapes.find(sh=>sh.id===selectedId);if(!s)return;
    const ns={...s,id:nextId} as Shape;
    if(ns.type==='rect'){ns.x+=gridSize;ns.y+=gridSize;}if(ns.type==='circle'){ns.cx+=gridSize;ns.cy+=gridSize;}
    setShapes(p=>[...p,ns]);setSelectedId(nextId);setNextId(p=>p+1);push('Duplicar');
  },[shapes,selectedId,nextId,gridSize,push]);
  const del=useCallback(()=>{if(selectedId!=null){setShapes(p=>p.filter(s=>s.id!==selectedId));setSelectedId(null);push('Excluir');}},[selectedId,push]);
  const exSim=useCallback(()=>exportSimMEMS(shapes,deviceW,deviceH,simPxW,simPxH,polarity),[shapes,deviceW,deviceH,simPxW,simPxH,polarity]);
  ...
  const exW=useCallback(()=>{const l=WAFER_SIZES.find(w=>w.d===waferDiam)?.label||'';exportWafer(shapes,deviceW,deviceH,waferDiam,streetW,polarity,marks,waferDPI,l);},[shapes,deviceW,deviceH,waferDiam,streetW,polarity,marks,waferDPI]);
  ...
  const doSave=useCallback(()=>saveProject({version:3,name:projectName,shapes,layers,deviceW,deviceH,gridSize,polarity,simPxW,simPxH,waferDiam,streetW,marks,waferDPI}),[projectName,shapes,layers,deviceW,deviceH,gridSize,polarity,simPxW,simPxH,waferDiam,streetW,marks,waferDPI]);
  const doLoad=useCallback(async(f:File)=>{try{const d=await loadProject(f);setShapes(d.shapes||[]);setLayers(d.layers||DL);setDeviceW(d.deviceW||500);setDeviceH(d.deviceH||500);setGridSize(d.gridSize||25);setPolarity(d.polarity||'dark');setSimPxW(d.simPxW||256);setSimPxH(d.simPxH||256);setWaferDiam(d.waferDiam||100000);setStreetW(d.streetW||200);setMarks(d.marks||DEFAULT_MARKS);setWaferDPI(d.waferDPI||2540);setProjectName(d.name||'Carregado');setSelectedId(null);setNextId(Math.max(...(d.shapes||[]).map(s=>s.id),0)+1);setHistory([{shapes:JSON.parse(JSON.stringify(d.shapes||[])),label:'Carregar'}]);setHi(0);}catch(e){alert('Erro: '+(e as Error).message);}},[]);
  ...
  return <div style={{width:'100%',height:'100vh',display:'flex',flexDirection:'column',background:'var(--bg)',overflow:'hidden',userSelect:'none'}}>
    ...
    <div style={{display:'flex',alignItems:'center',height:46,padding:'0 14px',background:'var(--panel)',borderBottom:'1px solid var(--border)',gap:8,flexShrink:0}}>
      ...
      <div style={{flex:1}}/>
      <span style={{fontSize:9,color:'var(--text-faint)',letterSpacing:'0.3px'}}>© {COPYRIGHT_HOLDER}</span>
      <div style={{width:1,height:24,background:'var(--border)'}}/>
    ...
    <div style={{display:'flex',flex:1,overflow:'hidden'}}>
      <Toolbar tool={tool} setTool={setTool} selectedId={selectedId} onDuplicate={dup} onDelete={del} onUndo={undo} onRedo={redo} canUndo={hi>0} canRedo={hi<history.length-1} onExportSimMEMS={exSim}/>
      <Canvas shapes={shapes} setShapes={setShapes} selectedId={selectedId} setSelectedId={setSelectedId} vp={vp} setVp={setVp} tool={tool} deviceW={deviceW} deviceH={deviceH} gridSize={gridSize} showGrid={showGrid} snapOn={snapOn} polarity={polarity} layers={layers} activeLayer={activeLayer} nextId={nextId} setNextId={setNextId} onShapeCreated={()=>push('Criar')} onShapeMoved={()=>push('Mover')} theme={theme}/>
      <RightPanel deviceW={deviceW} setDeviceW={setDeviceW} deviceH={deviceH} setDeviceH={setDeviceH} gridSize={gridSize} setGridSize={setGridSize} selectedShape={shapes.find(s=>s.id===selectedId)} selectedId={selectedId} shapes={shapes} setShapes={setShapes} layers={layers} setLayers={setLayers} activeLayer={activeLayer} setActiveLayer={setActiveLayer} simPxW={simPxW} setSimPxW={setSimPxW} simPxH={simPxH} setSimPxH={setSimPxH} polarity={polarity} waferDiam={waferDiam} setWaferDiam={setWaferDiam} streetW={streetW} setStreetW={setStreetW} marks={marks} setMarks={setMarks} waferDPI={waferDPI} setWaferDPI={setWaferDPI} onExportSimMEMS={exSim} onExportSimMEMSPerLayer={exSimL} onOpenWaferModal={()=>setWaferModal(true)}/>
    </div>

    {/* ─── Footer ─── */}
    <div style={{
      display:'flex', alignItems:'center', justifyContent:'center', gap:16,
      height:22, background:'var(--panel)', borderTop:'1px solid var(--border)',
      flexShrink:0, fontSize:9, color:'var(--text-faint)', letterSpacing:'0.3px',
    }}>
      <span>MaskCAD v{APP_VERSION}</span>
      <span>·</span>
      <span>© {COPYRIGHT_YEAR} {COPYRIGHT_HOLDER}. All rights reserved.</span>
    </div>
    ...
    {/* ─── About Modal ─── */}
    {aboutOpen&&(
      <div style={{position:'fixed',inset:0,background:'var(--modal-bg)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000,backdropFilter:'blur(6px)'}} onClick={()=>setAboutOpen(false)}>
        <div className="fade-in" onClick={e=>e.stopPropagation()} style={{background:'var(--panel)',borderRadius:12,border:'1px solid var(--border)',padding:32,maxWidth:420,width:'90vw',boxShadow:'0 24px 64px rgba(0,0,0,0.4)',textAlign:'center'}}>
          <div style={{display:'flex',justifyContent:'flex-end',marginBottom:8}}>
            <button onClick={()=>setAboutOpen(false)} style={{background:'none',border:'none',color:'var(--text-dim)',cursor:'pointer',padding:4}}><X size={18}/></button>
          </div>
          <Hexagon size={36} color="var(--accent)" strokeWidth={2} style={{margin:'0 auto 12px'}}/>
          <h2 style={{fontSize:20,fontWeight:700,margin:0,color:'var(--text)',letterSpacing:'2px'}}>
            MASK<span style={{color:'var(--accent)'}}>CAD</span>
          </h2>
          <p style={{fontSize:12,color:'var(--text-dim)',marginTop:4}}>v{APP_VERSION}</p>
          ...
          <p style={{fontSize:13,color:'var(--text)',lineHeight:1.7}}>
            Editor de máscaras litográficas para projeto e fabricação de dispositivos semicondutores e MEMS.
          </p>
          <p style={{fontSize:13,color:'var(--text)',lineHeight:1.7,marginTop:8}}>
            Exportação binária para simulação (simMEMS) e geração de fotolitos com step-and-repeat automático, marcas de alinhamento e contorno de die.
          </p>
          ...
          <p style={{fontSize:15,fontWeight:700,color:'var(--accent)',marginTop:4,letterSpacing:'0.5px'}}>
            {COPYRIGHT_HOLDER}
          </p>
          <p style={{fontSize:10,color:'var(--text-faint)',marginTop:12}}>
            © {COPYRIGHT_YEAR} {COPYRIGHT_HOLDER}. Todos os direitos reservados.
          </p>
        </div>
      </div>
    )}
