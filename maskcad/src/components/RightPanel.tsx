...
import { Eye, EyeOff, Download, ChevronDown, ChevronRight, Trash2, Edit3, Layers, Package, Target, Crosshair as XI, Box } from 'lucide-react';
import { Shape, Layer, Polarity, AlignMarkStyle, LayerPurpose, MarksConfig, WAFER_SIZES, LAYER_PURPOSE_LABELS, LAYER_PALETTE, calcDiePadding } from '../types';
...
function Toggle({value,onChange,label}:{value:boolean;onChange:(v:boolean)=>void;label:string}){
  return <label style={{display:'flex',alignItems:'center',gap:8,fontSize:11,color:'var(--text)',cursor:'pointer'}}>
    <div onClick={()=>onChange(!value)} style={{width:32,height:18,borderRadius:9,cursor:'pointer',background:value?'var(--accent)':'var(--border)',position:'relative',transition:'background 0.15s'}}>
      <div style={{width:14,height:14,borderRadius:7,background:'#fff',position:'absolute',top:2,left:value?16:2,transition:'left 0.15s'}}/>
    </div>
    <span style={{color:value?'var(--text)':'var(--text-dim)'}}>{label}</span>
  </label>;
}
function Sub({title,icon,children,open:defaultOpen=true}:{title:string;icon:React.ReactNode;children:React.ReactNode;open?:boolean}){
  const[o,setO]=useState(defaultOpen);
  return <div style={{marginTop:8,borderRadius:'var(--radius)',border:'1px solid var(--border)',overflow:'hidden'}}>
    <div onClick={()=>setO(!o)} style={{display:'flex',alignItems:'center',gap:6,padding:'6px 8px',background:'var(--panel-light)',cursor:'pointer',fontSize:11,fontWeight:600,color:'var(--text-dim)'}}>
      {o?<ChevronDown size={11}/>:<ChevronRight size={11}/>}{icon}<span>{title}</span>
    </div>
    {o&&<div style={{padding:'8px 10px',display:'flex',flexDirection:'column',gap:6}}>{children}</div>}
  </div>;
}
function ColorPicker({color,onChange,onClose}:{color:string;onChange:(c:string)=>void;onClose:()=>void}){
  return <div className="fade-in" style={{position:'absolute',top:'100%',left:0,zIndex:50,background:'var(--panel)',border:'1px solid var(--border)',borderRadius:8,padding:10,boxShadow:'0 8px 24px rgba(0,0,0,0.3)',marginTop:4}} onClick={e=>e.stopPropagation()}>
    <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:4,marginBottom:8}}>
      {LAYER_PALETTE.map(c=><button key={c} onClick={()=>{onChange(c);onClose();}} style={{width:26,height:26,borderRadius:4,border:c===color?'2px solid var(--text)':'2px solid transparent',background:c,cursor:'pointer'}}/>)}
    </div>
    <label style={{display:'flex',alignItems:'center',gap:6,fontSize:11,color:'var(--text-dim)'}}>
      <input type="color" value={color} onChange={e=>onChange(e.target.value)} style={{width:28,height:22,border:'none',cursor:'pointer',background:'transparent'}}/>
      <input type="text" value={color} onChange={e=>{if(/^#[0-9a-fA-F]{6}$/.test(e.target.value))onChange(e.target.value);}} style={{flex:1,background:'var(--input-bg)',border:'1px solid var(--border)',borderRadius:4,color:'var(--text)',padding:'3px 6px',fontSize:11,fontFamily:'var(--font-mono)',outline:'none'}}/>
    </label>
  </div>;
}
function LayerRow({layer,isActive,onActivate,onUpdate,onDelete,shapeCount,canDelete}:{layer:Layer;isActive:boolean;onActivate:()=>void;onUpdate:(p:Partial<Layer>)=>void;onDelete:()=>void;shapeCount:number;canDelete:boolean}){
  ...
  return <div style={{marginBottom:3,borderRadius:'var(--radius)',border:isActive?'1px solid var(--border-light)':'1px solid transparent',background:isActive?'var(--panel-light)':'transparent'}}>
    <div style={{display:'flex',alignItems:'center',gap:6,padding:'6px 8px',cursor:'pointer'}} onClick={onActivate}>
      ...
      <div style={{position:'relative'}}><button onClick={e=>{e.stopPropagation();setCp(!cp);}} style={{width:14,height:14,borderRadius:3,background:layer.color,border:'1px solid var(--border)',cursor:'pointer',boxShadow:isActive?`0 0 6px ${layer.color}50`:'none'}}/>{cp&&<ColorPicker color={layer.color} onChange={c=>onUpdate({color:c})} onClose={()=>setCp(false)}/>}</div>
    ...
    {exp&&<div className="fade-in" style={{padding:'4px 8px 8px 28px',display:'flex',flexDirection:'column',gap:6}}>
      ...
      <label style={{display:'flex',alignItems:'center',gap:6,fontSize:11}}><span style={{minWidth:58,color:'var(--text-dim)'}}>Opacidade</span><input type="range" min={10} max={100} value={layer.opacity} onChange={e=>onUpdate({opacity:Number(e.target.value)})} onClick={e=>e.stopPropagation()} style={{flex:1,accentColor:layer.color,height:3,cursor:'pointer'}}/><span style={{fontSize:10,fontFamily:'var(--font-mono)',color:'var(--text-faint)',minWidth:28,textAlign:'right'}}>{layer.opacity}%</span></label>
...
interface Props {
  ...
  selectedShape:Shape|undefined;selectedId:number|null;shapes:Shape[];setShapes:React.Dispatch<React.SetStateAction<Shape[]>>;
...
export default function RightPanel(p:Props){
  const{deviceW,setDeviceW,deviceH,setDeviceH,gridSize,setGridSize,selectedShape,selectedId,shapes,setShapes,
    ...
    onExportSimMEMS,onExportSimMEMSPerLayer,onOpenWaferModal}=p;
  ...
  const sL=layers.find(l=>selectedShape&&l.id===selectedShape.layer);
  ...
  return <div style={{width:275,display:'flex',flexDirection:'column',background:'var(--panel)',borderLeft:'1px solid var(--border)',flexShrink:0,overflow:'hidden'}}>
    ...
    {tab==='design'&&<>
      ...
      {selectedShape&&<>
        <div style={sec}>Forma Selecionada{sL&&<span style={{fontSize:9,padding:'1px 6px',borderRadius:10,background:sL.color+'30',color:sL.color,fontWeight:600}}>{sL.name}</span>}</div>
        <div style={{display:'flex',flexDirection:'column',gap:6}}>
          {selectedShape.type==='rect'&&<><NumInput label="X" value={selectedShape.x} onChange={v=>setShapes(q=>q.map(s=>s.id===selectedId?{...s,x:v}:s))}/><NumInput label="Y" value={selectedShape.y} onChange={v=>setShapes(q=>q.map(s=>s.id===selectedId?{...s,y:v}:s))}/><NumInput label="Largura" value={selectedShape.w} onChange={v=>setShapes(q=>q.map(s=>s.id===selectedId?{...s,w:Math.max(1,v)}:s))}/><NumInput label="Altura" value={selectedShape.h} onChange={v=>setShapes(q=>q.map(s=>s.id===selectedId?{...s,h:Math.max(1,v)}:s))}/></>}
          {selectedShape.type==='circle'&&<><NumInput label="Centro X" value={selectedShape.cx} onChange={v=>setShapes(q=>q.map(s=>s.id===selectedId?{...s,cx:v}:s))}/><NumInput label="Centro Y" value={selectedShape.cy} onChange={v=>setShapes(q=>q.map(s=>s.id===selectedId?{...s,cy:v}:s))}/><NumInput label="Raio" value={selectedShape.r} onChange={v=>setShapes(q=>q.map(s=>s.id===selectedId?{...s,r:Math.max(1,v)}:s))}/></>}
          <label style={{display:'flex',alignItems:'center',gap:6,fontSize:11,marginTop:4}}><span style={{minWidth:64,color:'var(--text-dim)'}}>Camada</span><select value={selectedShape.layer} onChange={e=>setShapes(q=>q.map(s=>s.id===selectedId?{...s,layer:Number(e.target.value)}:s))} style={sel}>{layers.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select></label>
        </div>
      </>}
      <div style={sec}>Atalhos</div>
      <div style={{fontSize:11,color:'var(--text-dim)',lineHeight:2}}>
        {[['R','Retângulo'],['C','Círculo'],['V','Selecionar'],['G','Grade'],['Del','Excluir'],['Ctrl+Z','Desfazer'],['Ctrl+⇧+Z','Refazer'],['Alt+Drag','Pan'],['Scroll','Zoom']].map(([k,l])=><div key={k} style={{display:'flex',gap:8,alignItems:'center'}}><kbd style={{background:'var(--kbd-bg)',padding:'1px 6px',borderRadius:3,fontSize:10,fontFamily:'var(--font-mono)',border:'1px solid var(--border)',minWidth:32,textAlign:'center'}}>{k}</kbd><span>{l}</span></div>)}
      </div>
    </>}
    ...
    {tab==='export'&&<>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:6}}>
        ...
        <div style={{fontSize:10,color:'var(--text-faint)',fontFamily:'var(--font-mono)',padding:'5px 8px',background:'var(--panel-light)',borderRadius:'var(--radius)',lineHeight:1.8}}>
          {calcDiePositions(waferDiam, deviceW, deviceH, streetW, marks).length} dies na lâmina<br/>
          Footprint do die: {totalW.toFixed(0)} × {totalH.toFixed(0)} µm<br/>
          <span style={{fontSize:9}}>({deviceW}×{deviceH} dispositivo + {pad.toFixed(0)} µm de padding)</span>
        </div>
      </div>

      <div style={sec}><Target size={11}/> Marcas de Alinhamento</div>

      <Sub title="Contorno do Die (Frame)" icon={<Box size={11}/>}>
        <Toggle value={marks.outline.enabled} onChange={v=>uO({enabled:v})} label="Ativar contorno"/>
        {marks.outline.enabled&&<>
          <NumInput label="Espessura" value={marks.outline.width} onChange={v=>uO({width:v})}/>
          <NumInput label="Distância" value={marks.outline.margin} onChange={v=>uO({margin:v})}/>
          <div style={{fontSize:9,color:'var(--text-faint)'}}>Gap entre a borda do dispositivo e o frame</div>
        </>}
      </Sub>

      <Sub title="Marcas do Die" icon={<Target size={11}/>}>
        <Toggle value={marks.die.enabled} onChange={v=>uD({enabled:v})} label="Ativar marcas no die"/>
        {marks.die.enabled&&<>
          <label style={{display:'flex',alignItems:'center',gap:6,fontSize:11}}><span style={{minWidth:64,color:'var(--text-dim)'}}>Estilo</span><MSel value={marks.die.style} onChange={v=>uD({style:v})}/></label>
          <NumInput label="Tamanho" value={marks.die.size} onChange={v=>uD({size:v})}/>
          <NumInput label="Offset" value={marks.die.offset} onChange={v=>uD({offset:v})}/>
          <div style={{fontSize:9,color:'var(--text-faint)'}}>Distância do frame (ou do dispositivo) ao centro da marca</div>
          <label style={{display:'flex',alignItems:'center',gap:6,fontSize:11}}><span style={{minWidth:64,color:'var(--text-dim)'}}>Posições</span>
            <select value={marks.die.positions} onChange={e=>uD({positions:e.target.value as any})} style={sel}><option value="corners">Cantos (4)</option><option value="edges">Arestas (4)</option><option value="corners+edges">Todos (8)</option></select>
          </label>
        </>}
      </Sub>

      <Sub title="Marcas do Wafer" icon={<XI size={11}/>}>
        <Toggle value={marks.wafer.enabled} onChange={v=>uW({enabled:v})} label="Ativar marcas do wafer"/>
        {marks.wafer.enabled&&<>
          <label style={{display:'flex',alignItems:'center',gap:6,fontSize:11}}><span style={{minWidth:64,color:'var(--text-dim)'}}>Estilo</span><MSel value={marks.wafer.style} onChange={v=>uW({style:v})}/></label>
          <NumInput label="Tamanho" value={marks.wafer.size} onChange={v=>uW({size:v})}/>
          <label style={{display:'flex',alignItems:'center',gap:6,fontSize:11}}><span style={{minWidth:64,color:'var(--text-dim)'}}>Disposição</span>
            <select value={marks.wafer.placement} onChange={e=>uW({placement:e.target.value as any})} style={sel}><option value="auto4">4 cardeais</option><option value="auto8">8 pontos</option><option value="manual">Manual</option></select>
          </label>
          <label style={{display:'flex',alignItems:'center',gap:6,fontSize:11}}><span style={{minWidth:64,color:'var(--text-dim)'}}>Posição</span>
            <input type="range" min={50} max={98} value={marks.wafer.radiusFraction*100} onChange={e=>uW({radiusFraction:Number(e.target.value)/100})} style={{flex:1,accentColor:'var(--accent)',height:3,cursor:'pointer'}}/>
            <span style={{fontSize:10,fontFamily:'var(--font-mono)',color:'var(--text-faint)',minWidth:28,textAlign:'right'}}>{(marks.wafer.radiusFraction*100).toFixed(0)}%</span>
          </label>
        </>}
      </Sub>
